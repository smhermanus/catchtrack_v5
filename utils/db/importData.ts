...
export class DataImporter {
    ...
    private validateQuotaData(quota: QuotaRow): void {
        ...
        if (!quota.marineResources || !Array.isArray(quota.marineResources)) missingFields.push('marineResources');
        ...
        if (quota.warningThreshold !== undefined && (quota.warningThreshold < 0 || quota.warningThreshold > 100)) {
            throw new Error('Warning threshold must be between 0 and 100');
        }
        if (quota.criticalThreshold !== undefined && (quota.criticalThreshold < 0 || quota.criticalThreshold > 100)) {
    ...
    async importRightsholders(rightsholders: RightsholderRow[], userIdMap: Map<string, number>): Promise<void> {
        try {
            await this.prisma.$transaction(async (prisma) => {
                for (const rightsholder of rightsholders) {
                    try {
                        ...
                        throw new Error(`Failed to create rightsholder ${rightsholder.registrationNumber}: ${errorMessage}`);
                    }
                }
            });
        } catch (err: unknown) {
