...
class DataUpserter {
  ...
  async readExcelFile(filePath: string): Promise<Record<string, any>[]> {
    try {
      ...
      if (rows.length > 1) {
        ...
        console.log(
          '\nSample Row (before conversion):',
          sampleRow.reduce((obj: Record<string, any>, val: any, idx: number) => {
            obj[headers[idx]] = val;
            return obj;
          }, {})
        );
  ...
  async upsertAllData(quotasFilePath: string, adminUserId: number): Promise<void> {
    try {
      ...
      console.log('\nStarting quota upsert...');
      const validatedQuotas: QuotaRow[] = rawQuotas.map((quota, index) =>
        this.validateQuota(quota, index)
      );
