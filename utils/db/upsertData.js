...
class DataUpserter {
  ...
  validateUser(user, index) {
    ...
    const validRoles = ['SYSTEMADMINISTRATOR', 'USER', 'RIGHTSHOLDER', 'INSPECTOR'];
    ...
    if (!validRoles.includes(role)) {
      throw new Error(
        `Row ${index + 2}: Invalid role '${user.role}'. Must be one of: ${validRoles.join(', ')}`
      );
    }

    if (!validStatuses.includes(status)) {
      throw new Error(
        `Row ${index + 2}: Invalid status '${user.status}'. Must be one of: ${validStatuses.join(', ')}`
      );
    }
